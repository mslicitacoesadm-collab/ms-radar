...
db = Database()
hero('Alertas Inteligentes', 'Transforme busca recorrente em rotina pronta: perfil, filtro e radar por nicho.')
...
modalidades = {0: 'Todas'} | MODALIDADES
...
with st.form('alert_form'):
    ...
    with c2:
        modalidade = st.selectbox('Modalidade', options=list(modalidades.keys()), format_func=lambda x: modalidades[x])
        ...
        email = st.text_input('Canal e-mail (opcional)')
        telegram = st.text_input('Chat ID Telegram (opcional)')
    somente_abertas = st.toggle('Somente oportunidades com prazo aberto', value=True)
    submitted = st.form_submit_button('Salvar alerta')

if submitted:
    if not nome.strip() or not termos.strip():
        st.error('Informe nome e ao menos um termo-chave.')
    else:
        db.save_alert_profile(
            {
                ...
                'canal_telegram_chat_id': telegram.strip() or None,
            }
        )
...
for alert in db.list_alert_profiles():
    ...
    st.caption(f"UF: {alert['uf_sigla'] or 'todas'} · Município: {alert['municipio_nome'] or 'todos'} · Email: {alert['canal_email'] or '—'}")
